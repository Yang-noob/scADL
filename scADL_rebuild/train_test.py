...
from read_datasets import CNN_Dataset
from torch.utils.data import DataLoader
import torch
from models import CNN_test
import pandas as pd
...
# print(label1_dict)
# print(lab)
dataset1 = dp.capitalize_genes_name(dataset1)
dataset1 = dp.filt_duplicate_rows(dataset1)
data_index = dataset1.index
sorted_index = sorted(set(data_index))
dataset1 = dataset1.loc[sorted_index,]
...
tensor_data = dip.mySVD(tensor_data, features=10000)
...
train_dataset = CNN_Dataset(tensor_data, tensor_label, percentage=0.8, train=True, transform=True)
train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)

'''使用Dataset和Dataloader加载验证集，percentage为验证集占比'''
test_dataset = CNN_Dataset(tensor_data, tensor_label, percentage=0.2, train=False, transform=True)
...
# 实例化网络模型
model = CNN_test(dropout=0.05, total_number_types=num_class)
